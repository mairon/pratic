class CobrosFinancasController < ApplicationController
  ...
  def create              #
    ...
    respond_to do |format|
      if @cobros_financa.save
        format.html { redirect_to "/cobros/cobro_final/#{@cobros_financa.cobro_id}" }
  ...
  def update              #
    @cobros_financa = CobrosFinanca.find(params[:id])
    @cobros_financa.usuario_updated = current_user.id
    @cobros_financa.unidade_updated = current_unidade.id
